...
def test_sft_uses_preformatted_jsonl_without_overwriting_it(tmp_path: Path) -> None:
    preformatted_path = tmp_path / "mixed.jsonl"
    original_content = (
        json.dumps(
            {
                "uid": "trajectory:0",
                "db_id": "music",
                "prompt": "repair this SQL",
                "completion": "SELECT Name FROM Singer",
                "source_type": "trajectory_rewrite",
            }
        )
        + "\n"
    )
    preformatted_path.write_text(original_content, encoding="utf-8")
    unused_output_path = tmp_path / "should_not_be_written.jsonl"
    config = {
        "data": {"preformatted_sft_jsonl": str(preformatted_path)},
        "output": {"sft_jsonl": str(unused_output_path)},
    }

    selected_path = _prepare_sft_jsonl(config)

    assert selected_path == preformatted_path
    assert preformatted_path.read_text(encoding="utf-8") == original_content
    assert not unused_output_path.exists()
