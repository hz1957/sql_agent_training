...
# Rollout comparison C: S1 chain-final with rollout_n=8, batch=4, and at most 24 SQL actions per root.
# Launch from the outer workspace root:
#   bash sql_agent_training/scripts/run_verl_grpo_qwen25_coder_14b_h100_4gpu_rollout_cmp_s1_n8_bs4.sh
...
RUN_NAME="${RUN_NAME:-verl_grpo_rollout_cmp_s1_chain_n8_base_ratio_gold3200_d1137_r463_lr5e5_r32_14b_h100_4gpu_${TOTAL_TRAINING_STEPS}step_bs4_kl01_ep2_t10_turn3_g09_dyn16k_rmpad_seed${DATA_SEED}_$(date +%Y%m%d_%H%M%S)}"
...
(
  ...
  echo "ROLLOUT_COMPARISON_CONFIG scheme=s1_chain_final base_model=${SFT_BASE_MODEL} batch_size=4 rollout_n=8 max_turns=3 max_actions_per_root=24 kl_coef=0.01 ppo_epochs=2 temperature=1.0 gamma=0.9 dynamic_bsz=True remove_padding=True token_cap_per_gpu=${PPO_MAX_TOKEN_LEN_PER_GPU} rollout_max_num_seqs=8 rollout_max_num_batched_tokens=8192 data_seed=${DATA_SEED} rollout_seed=${ROLLOUT_SEED} layered_summon=True"
