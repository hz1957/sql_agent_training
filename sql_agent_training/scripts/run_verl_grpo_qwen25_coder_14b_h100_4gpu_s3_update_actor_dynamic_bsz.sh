...
#   bash sql_agent_training/scripts/run_verl_grpo_qwen25_coder_14b_h100_4gpu_s3_update_actor_dynamic_bsz.sh
...
RUN_NAME="${RUN_NAME:-verl_grpo_s3_update_actor_dynamic_bsz_14b_h100_4gpu_${TOTAL_TRAINING_STEPS}step_bs2_n20_ep2_$(date +%Y%m%d_%H%M%S)}"
...
(
  ...
  echo "UPDATE_ACTOR_BENCHMARK_CONFIG variant=dynamic_bsz_remove_padding steps=${TOTAL_TRAINING_STEPS} warmup=${WARMUP_STEPS} train_batch_size=2 ppo_mini_batch_size=2 ppo_micro_batch_size_per_gpu=1 dynamic_bsz=True remove_padding=True ppo_max_token_len_per_gpu=${PPO_MAX_TOKEN_LEN_PER_GPU} branch_n=4 beam_size=2 rollout_n=20 ppo_epochs=2 kl_coef=0.01 layered_summon=True"
