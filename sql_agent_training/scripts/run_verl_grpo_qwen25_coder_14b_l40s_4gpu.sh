...
SQL_AGENT_FIX_LAYERED_SUMMON=${SQL_AGENT_FIX_LAYERED_SUMMON:-1}
...
GRPO_TREE_PRUNE_ON_GOLD_REWARD=${GRPO_TREE_PRUNE_ON_GOLD_REWARD:-}
GRPO_TREE_PRUNE_ON_TERMINAL_PROXY=${GRPO_TREE_PRUNE_ON_TERMINAL_PROXY:-${GRPO_TREE_PRUNE_ON_GOLD_REWARD:-True}}
GRPO_TREE_STABLE_REQUEST_SEEDS=${GRPO_TREE_STABLE_REQUEST_SEEDS:-False}
...
LOAD_SQL_AGENT_EXTENSIONS=False
case "${SQL_AGENT_FIX_LAYERED_SUMMON}" in
  1|true|True|TRUE|yes|Yes|YES|on|On|ON) LOAD_SQL_AGENT_EXTENSIONS=True ;;
esac
if [[ "${GRPO_ADV_ESTIMATOR}" == "grpo_multi_step" || "${GRPO_ADV_ESTIMATOR}" == "grpo_tree" ]]; then
  LOAD_SQL_AGENT_EXTENSIONS=True
fi
if [[ "${LOAD_SQL_AGENT_EXTENSIONS}" == "True" ]]; then
...
export SQL_AGENT_FIX_LAYERED_SUMMON
...
export GRPO_TREE_PRUNE_ON_TERMINAL_PROXY
export GRPO_TREE_PRUNE_ON_GOLD_REWARD
export GRPO_TREE_STABLE_REQUEST_SEEDS
...
echo "verl TRAINER_USE_V1=${TRAINER_USE_V1}"
echo "verl TRAIN_BATCH_SIZE=${TRAIN_BATCH_SIZE} PPO_MINI_BATCH_SIZE=${PPO_MINI_BATCH_SIZE} PPO_EPOCHS=${PPO_EPOCHS} ROLLOUT_N=${ROLLOUT_N}"
echo "verl DATA_SEED=${DATA_SEED} ROLLOUT_SEED=${ROLLOUT_SEED}"
echo "verl FILTER_OVERLONG_PROMPTS=${FILTER_OVERLONG_PROMPTS} FILTER_OVERLONG_PROMPTS_WORKERS=${FILTER_OVERLONG_PROMPTS_WORKERS} DATALOADER_NUM_WORKERS=${DATALOADER_NUM_WORKERS}"
...
echo "verl SQL_AGENT_FIX_LAYERED_SUMMON=${SQL_AGENT_FIX_LAYERED_SUMMON}"
echo "verl ROLLOUT_ENABLE_PREFIX_CACHING=${ROLLOUT_ENABLE_PREFIX_CACHING} ROLLOUT_DISABLE_LOG_STATS=${ROLLOUT_DISABLE_LOG_STATS}"
...
if [[ -n "${ACTOR_CLIP_RATIO_LOW}" || -n "${ACTOR_CLIP_RATIO_HIGH}" ]]; then
  echo "verl ACTOR_CLIP_RATIO_LOW=${ACTOR_CLIP_RATIO_LOW:-<default>} ACTOR_CLIP_RATIO_HIGH=${ACTOR_CLIP_RATIO_HIGH:-<default>}"
fi
...
echo "verl GRPO_TREE_BRANCH_N=${GRPO_TREE_BRANCH_N} GRPO_TREE_BEAM_SIZE=${GRPO_TREE_BEAM_SIZE} GRPO_TREE_BEAM_TAU=${GRPO_TREE_BEAM_TAU} GRPO_TREE_BEAM_EPSILON_RANDOM=${GRPO_TREE_BEAM_EPSILON_RANDOM} GRPO_TREE_PRUNE_ON_TERMINAL_PROXY=${GRPO_TREE_PRUNE_ON_TERMINAL_PROXY}"
echo "verl GRPO_TREE_STABLE_REQUEST_SEEDS=${GRPO_TREE_STABLE_REQUEST_SEEDS}"
...
if [[ -n "${ACTOR_CLIP_RATIO_LOW}" ]]; then
  ACTOR+=(actor_rollout_ref.actor.clip_ratio_low="${ACTOR_CLIP_RATIO_LOW}")
fi
if [[ -n "${ACTOR_CLIP_RATIO_HIGH}" ]]; then
  ACTOR+=(actor_rollout_ref.actor.clip_ratio_high="${ACTOR_CLIP_RATIO_HIGH}")
fi
