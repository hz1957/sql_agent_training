...
class SQLiteTool:
    ...
    def execute(self, sqlite_path: str | Path, sql: str, *, copy_database: bool = True) -> SqlExecutionResult:
        ...
        try:
            ...
            return SqlExecutionResult(False, [], reason, time.monotonic() - start, sql, reason)
        if not safety.is_safe:
            return SqlExecutionResult(False, [], safety.reason, time.monotonic() - start, sql, safety.reason)
