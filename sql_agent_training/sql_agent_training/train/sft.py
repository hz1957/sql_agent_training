...
def _final_checkpoint_dir(config: dict) -> Path:
    """Return the directory that should contain the eval-ready SFT model."""

    return Path(config["output"]["checkpoint_dir"])


def _trainer_output_dir(config: dict) -> Path:
    ...
    return _final_checkpoint_dir(config) / "trainer_checkpoints"
...
def _run_transformers_training(config: dict, tokenizer, dataset: SftTorchDataset) -> None:
    ...
    args = TrainingArguments(
        output_dir=str(_trainer_output_dir(config)),
    ...
    trainer.train()
    final_checkpoint_dir = _final_checkpoint_dir(config)
