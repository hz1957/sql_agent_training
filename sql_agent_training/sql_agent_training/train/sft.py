...
def _configured_sft_jsonl(config: dict) -> Path:
    preformatted = config.get("data", {}).get("preformatted_sft_jsonl")
    if preformatted:
        return Path(preformatted)
    return Path(config["output"]["sft_jsonl"])


def _prepare_sft_jsonl(config: dict) -> Path:
    output_file = _configured_sft_jsonl(config)
    if config.get("data", {}).get("preformatted_sft_jsonl"):
        if not output_file.is_file():
            raise FileNotFoundError(f"Preformatted SFT JSONL does not exist: {output_file}")
        count = sum(1 for line in output_file.read_text(encoding="utf-8").splitlines() if line.strip())
        if count == 0:
            raise ValueError(f"Preformatted SFT JSONL is empty: {output_file}")
        print(f"Using {count} preformatted SFT records from {output_file}")
        return output_file
...
def _run_transformers_training(
    ...
    *,
    ...
    args = TrainingArguments(
        ...
        data_seed=int(training["data_seed"]) if training.get("data_seed") is not None else None,
